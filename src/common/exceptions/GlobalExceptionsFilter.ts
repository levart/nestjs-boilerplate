...
export class GlobalExceptionsFilter implements ExceptionFilter {
  public catch(exception: any, host: ArgumentsHost): void {
    ...
    console.error(
      `GlobalExceptionsFilter: ${clazz}: ${JSON.stringify(exception)}`,
    );
    ...
    if (clazz === 'BadRequestException') {
      ...
      if (exception.message.indexOf('duplicate key') >= -1) {
        response
          ...
          .json({ message: 'a similar record already exists' });
      }
    } else if (
      [
      ...
      console.error(
        `GlobalExceptionsFilter: Unknown exception class ${clazz}: ${JSON.stringify(
          exception,
        )}`,
      );
